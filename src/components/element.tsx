...
import { useSmartLazyLoad } from '@/lib/use-smart-lazy-load'
...
const Element = ({ element, showInfor = true }: Props) => {
  const { ref, enabled, done } = useSmartLazyLoad(element.id)

  const elementCodeQuery = useGetElementCodeQuery({
    id: element.id,
    enabled: enabled && !!element.id 
  })

  const elementCode = elementCodeQuery.data?.payload ?? { html: '', css: '' }
  const isDone = elementCodeQuery.isSuccess || elementCodeQuery.isError
  ...
  const showSkeleton = enabled && elementCodeQuery.isLoading
  return (
    <article
      ref={ref}
    ...
    >
      <div className='group relative overflow-hidden h-[250px] bg-[var(--dark)] rounded-lg min-h-[420px] grow'>
        {showSkeleton && (
          <div className='absolute inset-0 flex items-center justify-center bg-[var(--dark)]'>
        ...
        {enabled && <CodePreview isTailwind={element.isTailwind} html={elementCode.html} css={elementCode.css} />}
      </div>
      {showInfor && (
