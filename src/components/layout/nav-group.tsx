      ...
      <SidebarMenu>
        {items.map((item) => {
          ...
          if (state === 'collapsed' && !isMobile)
            return <SidebarMenuCollapsedDropdown key={key} role={role} item={item} href={href} />

          return <SidebarMenuCollapsible key={key} role={role} item={item} href={href} />
        })}
      </SidebarMenu>
    </SidebarGroup>
  )
...
function SidebarMenuCollapsible({ role, item, href }: { role: RoleType | undefined; item: NavCollapsible; href: string }) {
  const { setOpenMobile } = useSidebar()
  return (
