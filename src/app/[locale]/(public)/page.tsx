...
import { GetUsersResType } from '@/schemaValidations/user.schema'
...
const TopCreatorsCard = ({ user }: { user: any }) => {
  return (
    <Link
      href={`/profile/${user.id}`}
      className='flex gap-4 bg-dark-800 p-3 pl-5 rounded-xl items-center border-dark-600/80 border-2'
    >
      <span className='relative flex shrink-0 overflow-hidden rounded bg-black w-[48px] h-[48px]'>
      ...
      <div className='pr-3'>
        <div className='font-semibold max-w-[200px] font-display truncate text-lg text-offwhite'>{user.name}</div>
        <div className='flex items-center gap-3'>
          <div className=' text-gray-200 font-semibold py-1 rounded-full font-sm'>
            <span className='text-gray-400'>{user.approvedCount}</span> <span className='text-gray-400'>elements</span>
          </div>
          <div className='flex font-semibold gap-1.5 py-1 leading-none items-center rounded-full font-sm'>
            <svg viewBox='0 0 24 24' fill='none' className='w-5 h-5 text-indigo-400' xmlns='http://www.w3.org/2000/svg'>
              <path
...
export default async function Home(props: { params: Promise<{ locale: string }> }) {
  ...
  const now = new Date()
  const [elementResult, userResult] = await Promise.allSettled([
    wrapServerApi(() =>
  ...
  const userList: GetUsersResType['data'] = userResult.status === 'fulfilled' ? (userResult.value?.payload.data ?? []) : []
  return (
            ...
            <div className='p-5 pb-0 relative'>
              ...
              <div className='m-auto py-5 gap-3 max-w-[530px] w-full'>
                <div className='flex w-full relative z-[21] p-1 mx-auto  hover:scale-105 bg-gray-300 shadow-lg overflow-visible [&:has(:focus-visible)]:ring-4 rounded-2xl transition-transform [&:has(:focus-visible)]:ring-indigo-400 [&:has(:focus-visible)]:border-indigo-400'>
                  ...
                  <button className='flex items-center px-8 py-1.5 text-base gap-1.5 font-sans font-semibold transition-colors bg-indigo-600 hover:bg-indigo-700 text-offwhite group border-none cursor-pointer rounded-xl pointer'>
                    <span>Search</span>
                  </button>
                </div>
              </div>
        ...
        <section className='relative z-10 flex flex-col gap-4 px-4 mx-auto mb-24 sm:container sm:px-0'>
            ...
            <div className='bg-dark-800 min-h-[440px] md:min-h-[300px] min-w-[300px] w-full flex-1 rounded-3xl max-w-[700px] overflow-hidden border-dark-600/80 border-2'>
              <section className='relative z-10 flex items-end w-full h-full px-10 py-10 overflow-hidden '>
                ...
                >
                  <Image
                    className='w-full'
                    src='https://uiverse.io/assets/figma-CmpLUCXr.png'
                    alt='codeui'
                    width={0}
                    height={0}
                    sizes='100vw'
          ...
          <div className='mx-5 overflow-hidden border-2 bg-dark-800 rounded-3xl border-dark-600/80'>
            <div className='pt-10 pb-0 relative'>
              ...
              <div className='flex flex-col gap-[10px] mt-auto pb-6'>
                <Marquee pauseOnHover className='[--duration:40s]'>
                  {userList.slice(0, userList.length / 2).map((user) => (
                    <TopCreatorsCard key={user.id} user={user} />
                  ))}
                </Marquee>
                <Marquee reverse pauseOnHover className='[--duration:40s]'>
                  {userList.slice(userList.length / 2).map((user) => (
                    <TopCreatorsCard key={user.id} user={user} />
                  ))}
                </Marquee>
                <Marquee pauseOnHover className='[--duration:40s]'>
