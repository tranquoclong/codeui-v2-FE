...
const hasMore = commentsQuery.hasNextPage
  // const repliesQuery = useReplyListQuery(commentId, { page, limit })
  const addCommentMutation = useAddCommentMutation({ page:1, limit })
  const deleteCommentMutation = useDeleteCommentMutation()

  // const replyList = repliesQuery.data?.payload?.data ?? []
  // const hasMoreReplies = page < (repliesQuery.data?.payload?.totalPages || 1)
  ...
  const handleDeleteReply = async (replyId: number) => {
    if (!confirm('Bạn có chắc muốn xóa reply này?')) return
    try {
